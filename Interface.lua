...
function Interface.LoadVariables()
	...
	ContainerWindow.EnableAutoIgnoreCorpses =			Interface.LoadBoolean( "EnableAutoIgnoreCorpses", ContainerWindow.EnableAutoIgnoreCorpses ) 
...
function Interface.CreateWindows()
	...
	CreateWindow( "AdvancedStatusWindowHP", Interface.StatusWindowStyle  == 1 )
	CreateWindow( "AdvancedStatusWindowMANA",  Interface.StatusWindowStyle  == 1 )
	CreateWindow( "AdvancedStatusWindowSTAM",  Interface.StatusWindowStyle  == 1 )
	CreateWindow( "StatusWindow", Interface.StatusWindowStyle  == 0 )
...
function Interface.InterfaceInitialize() 
	...
	if( not Interface.InitialPositions) then
		...
		WindowClearAnchors("AdvancedStatusWindowHP")
		WindowAddAnchor("AdvancedStatusWindowHP", "bottomleft", "Root", "bottomleft", 0,0)
		
		WindowClearAnchors("AdvancedStatusWindowMANA")
		WindowAddAnchor("AdvancedStatusWindowMANA", "bottomright", "Root", "bottomright", 0,0)
		...
		WindowClearAnchors("AdvancedStatusWindowSTAM")
		WindowAddAnchor("AdvancedStatusWindowSTAM", "top", "Hotbar" .. Interface.MenuBar, "bottom", 0,0)
		local x, y= WindowGetOffsetFromParent("AdvancedStatusWindowSTAM")
		WindowClearAnchors("AdvancedStatusWindowSTAM")
		WindowSetOffsetFromParent("AdvancedStatusWindowSTAM", x,y)
		
		WindowClearAnchors("WarShield")
		WindowAddAnchor("WarShield", "bottomleft", "AdvancedStatusWindowSTAM", "bottomleft", -50, 50)
		local x, y= WindowGetOffsetFromParent("WarShield")
		WindowClearAnchors("WarShield")
		WindowSetOffsetFromParent("WarShield", x,y)
		
		WindowClearAnchors("AdvancedBuffEvil")
		WindowAddAnchor("AdvancedBuffEvil", "top", "AdvancedStatusWindowSTAM", "bottom", 115, 85)
		...
		WindowAddAnchor("AdvancedBuffGood", "top", "AdvancedStatusWindowSTAM", "bottom", -115, 85)
		...
		if DoesWindowNameExist("TipoftheDayWindow") then
			WindowClearAnchors("TipoftheDayWindow")
			WindowAddAnchor("TipoftheDayWindow", "center", "ResizeWindow", "center", 0, 0)
			local x, y= WindowGetOffsetFromParent("TipoftheDayWindow")
			WindowClearAnchors("TipoftheDayWindow")
			WindowSetOffsetFromParent("TipoftheDayWindow", x,y)
		end
...
function Interface.Update( timePassed )
	...
	ok, err = pcall(GumpsParsing.CheckGumpType, timePassed)	
	Interface.ErrorTracker(ok, err)
...
--		Debug.Print("Interface.GetMobileData")
...
function Interface.ItemUseRequest()
	...
	if not WindowData.ObjectInfo[GameData.UseRequests.UseItem] then
		RegisterWindowData(WindowData.ObjectInfo.Type, GameData.UseRequests.UseItem)
	end
	if not WindowData.ObjectInfo[GameData.UseRequests.UseItem] then
		GameData.UseRequests.UseItem = UserActionGetNextObjectId(GameData.UseRequests.UseItem)
		RegisterWindowData(WindowData.ObjectInfo.Type, GameData.UseRequests.UseItem)
	end
	...
	if itemData.containerId and itemData.containerId ~= 0 then
		if DoesWindowNameExist("ContainerWindow_"..itemData.containerId) then
			ContainerWindow.UpdateContents(itemData.containerId)
		end
	end
	...
	--Interface.ItemsCooldown[itemData.objectType][itemData.hueId] = 
	if ContainerWindow and itemData and itemData.containerId then
		
		local td = {func = function() ContainerWindow.UpdateUsesByID(itemData.containerId, GameData.UseRequests.UseItem) end, time = Interface.TimeSinceLogin + 2}
