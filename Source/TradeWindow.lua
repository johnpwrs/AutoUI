...
function TradeWindow.Initialize()
	...
	TradeWindow.totalGoldPlayer = 0
	TradeWindow.totalGoldOffered = 0

	LabelSetText(this.. "PlayerAcceptGoldLabel", GetStringFromTid(1079160) .. L":")
	LabelSetText(this.. "PlayerWaitingGoldLabel", GetStringFromTid(1079160) .. L":")
	
	LabelSetText(this.. "PlayerAcceptGoldAmount", L""..TradeWindow.totalGoldPlayer )
	LabelSetText(this.. "PlayerWaitingGoldAmount", L""..TradeWindow.totalGoldOffered)
...
function TradeWindow.UpdateContents(contId, win, force)
	...
	if (data and TradeWindow.TradeInfo[this] ~= nil ) then
		...
		if (updateFlag == true) then
			...
			for i = 1, #items do
				local amount = 0
				RegisterWindowData(WindowData.ObjectInfo.Type, items[i])
				local itemData = WindowData.ObjectInfo[items[i]]
				
				if itemData and (wstring.lower(itemData.name) == GetStringFromTid(1041361)) then -- a bank check
					local prop = ItemProperties.GetObjectPropertiesParamsForTid( items[i], 1074784,  "Trade window - bank check" )
					amount =  tonumber(tostring(wstring.gsub(prop[1], L",", L"")))
				end
				if itemData and itemData.objectType == 3821 then -- coins
					amount = itemData.quantity
				end
				if( TradeWindow.TradeInfo[this].containerId == contId ) then
					TradeWindow.totalGoldPlayer = TradeWindow.totalGoldPlayer + amount
					LabelSetText(this.. "PlayerAcceptGoldAmount", WindowUtils.AddCommasToNumber(TradeWindow.totalGoldPlayer ))
				elseif (TradeWindow.TradeInfo[this].containerId2 == contId) then
					TradeWindow.totalGoldOffered = TradeWindow.totalGoldOffered + amount
					LabelSetText(this.. "PlayerWaitingGoldAmount", WindowUtils.AddCommasToNumber(TradeWindow.totalGoldOffered))
				end
			end
			for i = 1, numItems do
