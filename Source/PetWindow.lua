...
function PetWindow.UpdateAnchors()
	...
	for key, value in pairsByKeys(PetWindow.SortedPet) do
		...
		if ( not IsMobile(mobileId) or MobileHealthBar.windowDisabled[mobileId]) and not MobileHealthBar.Handled[mobileId] then
			MobileHealthBar.CloseWindowByMobileId(mobileId)
			continue
		end
		if (value and MobilesOnScreen.IsPet(mobileId)) then
			...
			if (DoesWindowNameExist(windowName) and not MobileHealthBar.Handled[mobileId]) then
