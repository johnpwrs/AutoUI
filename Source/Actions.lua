...
function Actions.ObjectHandleContextMenu()
    ...
    for filter = 1, SettingsWindow.NUM_OBJHANDLE_FILTERS do
        ContextMenu.CreateLuaContextMenuItem(SettingsWindow.ObjectHandles[filter].tid,0,filter,filter,SystemData.Settings.GameOptions.objectHandleFilter == SettingsWindow.ObjectHandles[filter].id and not ObjectHandleWindow.ItemsOnly and not ObjectHandleWindow.LostItemsOnly)
    end    
...
function Actions.ImbueLast()
	if GumpsParsing.ParsedGumps[999059] then
		GumpsParsing.PressButton(999059, 2)
	else
