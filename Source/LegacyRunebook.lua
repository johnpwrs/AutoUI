...
LegacyRunebook.Hues = { malas = 1102, trammel = 10, tokuno = 1154, felucca = 81 }
...
LegacyRunebook.LegacyLabelColors= { malas={ r=75, g=75, b=75 }, trammel={ r=60, g=0, b=160 }, tokuno={ r=0, g=50, b=0 }, felucca={ r=10, g=90, b=90 } }
...
function LegacyRunebook.ResetDefaultIconText(data)
	...
	for i = 1, LegacyRunebook.NUM_DEFAULT_LABELS do
		...
		if (WindowData.SkillDynamicData[HealthBarManager.SpellSchools.MAGERY].TempSkillValue >= 660 and LegacyRunebook.KeymapLabel[i] == "GateTravel" ) then
			WindowSetShowing(windowName..LegacyRunebook.KeymapLabel[i], true)
		elseif (WindowData.SkillDynamicData[HealthBarManager.SpellSchools.MAGERY].TempSkillValue < 660 and LegacyRunebook.KeymapLabel[i] == "GateTravel" ) then
			WindowSetShowing(windowName..LegacyRunebook.KeymapLabel[i], false)
		end
		
		if (WindowData.SkillDynamicData[HealthBarManager.SpellSchools.CHIVALRY].TempSkillValue >= 150 and LegacyRunebook.KeymapLabel[i] == "Sacred" ) then
			WindowSetShowing(windowName..LegacyRunebook.KeymapLabel[i], true)
		elseif (WindowData.SkillDynamicData[HealthBarManager.SpellSchools.CHIVALRY].TempSkillValue < 150 and LegacyRunebook.KeymapLabel[i] == "Sacred" ) then
			WindowSetShowing(windowName..LegacyRunebook.KeymapLabel[i], false)
		end
