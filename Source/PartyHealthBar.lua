...
function PartyHealthBar.RedButton_OnLButtonUp()
	...
	if(mobileId) then
		local spellId = 0
		if (buttonDef == 1) then
			spellId = Interface.RedButton
		elseif (buttonDef == 2) then
			spellId = Interface.RedButton2
		elseif (buttonDef == 3) then
			spellId = Interface.RedButton3
		end
...
function PartyHealthBar.GreenButton_OnLButtonUp()
	...
	if(mobileId) then
		local spellId = 0
		if (buttonDef == 1) then
			spellId = Interface.GreenButton
		elseif (buttonDef == 2) then
			spellId = Interface.GreenButton2
		elseif (buttonDef == 3) then
			spellId = Interface.GreenButton3
		end
...
function PartyHealthBar.BlueButton_OnLButtonUp()
	...
	if(mobileId) then
		local spellId = 0
		if (buttonDef == 1) then
			spellId = Interface.BlueButton
		elseif (buttonDef == 2) then
			spellId = Interface.BlueButton2
		elseif (buttonDef == 3) then
			spellId = Interface.BlueButton3
		end
...
function PartyHealthBar.Button_OnRButtonUp()
	...
	if (string.find(SystemData.ActiveWindow.name, "Red")) then
		...
		local spellId = Interface.RedButton
		local icon, serverId, tid, desctid = GetAbilityData(spellId)
		if (not tid) then
			tid = 1011051
		end
	...
	elseif (string.find(SystemData.ActiveWindow.name, "Green")) then
		local buttonDef = Interface.LoadNumber("GreenButtonDef_" .. mobileId, Interface.GreenDef)
		local spellId = Interface.GreenButton
		local icon, serverId, tid, desctid = GetAbilityData(spellId)
		if (not tid) then
			tid = 1011051
		end
	...
	elseif (string.find(SystemData.ActiveWindow.name, "Blue")) then
		local buttonDef = Interface.LoadNumber("BlueButtonDef_" .. mobileId, Interface.BlueDef)
		local spellId = Interface.BlueButton
		local icon, serverId, tid, desctid = GetAbilityData(spellId)
		if (not tid) then
			tid = 1011051
		end
