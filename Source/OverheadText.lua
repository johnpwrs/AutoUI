...
function OverheadText.UpdateName(mobileId)
	...
	if(data and data.MobName ~= nil) then
		...
		if (name ~= "" and MobilesOnScreen.IsPet(mobileId)) then
			PetWindow.UpdatePet()
		end
		...
		if ( MobilesOnScreen.GetVisible() and name ~= "" and not MobilesOnScreen.IsPet(mobileId) and not MobileHealthBar.windowDisabled[mobileId]) then
			if (not MobilesOnScreen.MobilesSortReverse[mobileId]) then
