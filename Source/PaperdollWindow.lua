...
function PaperdollWindow.Shutdown()
	...
	if (WindowData.MobileStatus[paperdollId] or WindowData.MobileStatus[0][paperdollId]) and WindowData.PlayerStatus.PlayerId ~= paperdollId then
...
function PaperdollWindow.UpdatePaperdoll(windowName,paperdollId)
	...
	if( paperdollId == WindowGetId(windowName) ) then
		...
		if textureData ~= nil then
			...
			if texWidth ~= newWidth or texHeight ~= newHeight then
				...
				local tabTextureNames = { tabNames[1].."Texture" }
				local tabLegacyTextureNames = { tabNames[1].."LegacyTexture" }
	...
	local playerId = WindowData.PlayerStatus.PlayerId
	if(WindowGetShowing("PropertiesInfoWindow") and not PropertiesInfo.InSearch and wstring.len(TextEditBoxGetText("PropertiesInfoWindowSearchBox"))<1 ) then
...
function PaperdollWindow.UpdateCharProfile()
    ...
    if( id == WindowData.CharProfile.Id ) then
       local tabName = ProfileWindow.WindowName        
        if( id == WindowData.PlayerStatus.PlayerId ) then
